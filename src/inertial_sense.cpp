...
void InertialSenseROS::configure_data_streams()
{
    ...
    imu_pub = gnss_inertial_nh->advertise<sensor_msgs::Imu>("imu", 1, false);              //Livox
    gnss_pub = gnss_inertial_nh->advertise<sensor_msgs::NavSatFix>("navsatfix", 1, false); //Livox
...
{
    int NMEA_rate = nh_private_.param<int>("NMEA_rate", 250);
    ascii_msgs_t msgs = {};
    msgs.options = 0x01; // output on serial 0

    msgs.gprmc = NMEA_rate;
...
void InertialSenseROS::INS2_callback(const ins_2_t *const msg)
{
    ...
    imu_msg.header.frame_id = "uINS";
    imu_msg.header.stamp.sec = msg->timeOfWeek - 18;
